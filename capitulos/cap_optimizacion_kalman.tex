...
\subsection{Reformulación Matemática: EKF con Sesgo Dual Dependiente del Rango}
...
\subsection{Filtro de Kalman Extendido Iterado (IEKF)}
...
\subsection{Validación Empírica del Filtro Optimizado}
...
Para contrastar el modelo propuesto con una técnica no recursiva diseñada específicamente para condiciones de propagación adversas, la Figura \ref{fig:comparativa_iekf_multi_nlos} presenta las trayectorias obtenidas mediante el IEKF de estado aumentado y la multilateración ponderada NLOS sobre el mismo conjunto de datos correspondiente a la cadera del sujeto 2. Esta comparación permite evaluar si la estimación conjunta de los sesgos conserva un desempeño competitivo frente a un método que asigna pesos reducidos a las mediciones identificadas como NLOS.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/comparacion_iekf_multi_nlos.png}
    \caption{Comparación de trayectorias entre el IEKF con estado aumentado y la multilateración ponderada NLOS. Escenario de prueba: sensor en la cadera del sujeto 2.}
    \label{fig:comparativa_iekf_multi_nlos}
\end{figure}

La Figura \ref{fig:comparativa_iekf_multi_nlos} muestra un desempeño global prácticamente equivalente. El IEKF obtiene un MAE ligeramente menor, i.e., 0.4253 m frente a 0.4264 m, y un error máximo de 0.8224 m frente a 0.8334 m. Por su parte, la multilateración ponderada NLOS presenta una mediana menor, i.e., 0.4210 m frente a 0.4730 m, y una desviación estándar inferior, i.e., 0.1763 m frente a 0.2001 m. Por tanto, no existe un dominio absoluto de un método sobre el otro: el IEKF ofrece una ventaja marginal en exactitud promedio y error extremo, mientras que Multi-NLOS produce una distribución de errores ligeramente más estable.
