\chapter{ANÁLISIS DE REQUERIMIENTOS, DISEÑO DEL SISTEMA E IMPLEMENTACIÓN}
...
El diseño del sistema abarca la configuración detallada del hardware, la arquitectura de software empleada para la adquisición y procesamiento de datos, y la definición de los escenarios experimentales donde se evaluará el desempeño.

\subsection{Arquitectura de Hardware}
\label{subsec:arquitectura_hardware}

Los dispositivos UWB operan con el firmware PANS (Positioning and Networking Stack) de Qorvo, implementando la técnica TWR para estimación de distancia en la banda de frecuencia central de 6.5 GHz. Los dispositivos se configuran con los siguientes parámetros operacionales para optimizar el compromiso entre exactitud, alcance y consumo:
...
\subsubsection{Escenario Exterior}
...
\subsubsection{Escenario Interior}
...
Para cada combinación de ubicación corporal, condición de canal, escenario y distancia, se registraron \textbf{250 mediciones de distancia estimada} por el sistema UWB.
...
El protocolo experimental implementado en cada sesión se estructuró de manera sistemática. Inicialmente, se procedió con la preparación del sistema y la marcación del grid. Antes de iniciar las pruebas, se ejecutó una calibración inicial en línea de vista (LOS) situando el nodo móvil en un trípode sin la presencia del sujeto.

Una vez validado el sistema, se procedió a la instrumentación del sujeto. La adquisición de datos comenzó en condición LOS para cada punto de distancia (1 a 13 m). Posteriormente, se realizó la adquisición de datos en condición NLOS, donde el sujeto rotaba 180 grados. Al completar las mediciones en una posición corporal específica, se efectuó el cambio de ubicación del nodo móvil, repitiendo el ciclo. Una vez cubiertas todas las ubicaciones, se replicó el procedimiento en el segundo escenario.

\textbf{Condiciones de Control:}
\begin{itemize}
\item Horarios de mínima actividad para reducir interferencias.
\item Verificación de ausencia de cambios significativos en el entorno.
\item Registro de condiciones ambientales (temperatura, humedad).
\item Calibración inicial LOS en cada sesión.
\end{itemize}
...
\begin{itemize}
\item \textbf{Error Absoluto Medio (MAE):} $\text{MAE} = \frac{1}{N} \sum_{j=1}^{N} |e_j|$
\item \textbf{Raíz del Error Cuadrático Medio (RMSE):} $\text{RMSE} = \sqrt{\frac{1}{N} \sum_{j=1}^{N} e_j^2}$
...
Para cada condición experimental se caracteriza la distribución estadística de los errores observados, evaluando distribuciones teóricas (Normal, Log-normal, Gamma) mediante pruebas de bondad de ajuste (Kolmogorov-Smirnov, AIC, BIC).
