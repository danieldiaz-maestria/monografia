...
Con el objetivo de caracterizar formalmente el comportamiento estocástico del error de medición y validar la hipótesis de no normalidad sugerida en las secciones anteriores, se realizó un análisis de ajuste de distribuciones probabilísticas sobre el conjunto total de datos experimentales ($N=101,552$ muestras). Se evaluaron seis distribuciones teóricas candidatas: Normal, Log-normal, Gamma, Weibull, Exponencial y Uniforme. La bondad de ajuste se determinó mediante el Criterio de Información de Akaike (AIC) y el Criterio de Información Bayesiano (BIC), donde valores menores indican un mejor compromiso entre la fidelidad del ajuste y la complejidad del modelo.

\subsection{Ajuste Global de Datos}

El análisis del conjunto global de datos confirma de manera concluyente que la distribución Normal es inadecuada para modelar el error de distancia en sistemas UWB bajo condiciones mixtas (LOS/NLOS). El análisis de bondad de ajuste revela que la distribución Normal presenta valores de AIC significativamente superiores (peor ajuste) en comparación con distribuciones asimétricas como Gamma o Log-normal.
...
Al segregar los datos por condición de propagación, se revelan diferencias fundamentales en la estructura probabilística del error.
...
\textbf{Condición de No Línea de Vista (NLOS):} En condiciones NLOS ($N=44,981$), la distribución del error sufre una transformación drástica. La Figura~\ref{fig:ajuste_nlos} muestra una dispersión mucho mayor y una cola pesada significativa hacia la derecha. El análisis de AIC indica que la distribución Log-normal ofrece el mejor ajuste para estos datos, superando ligeramente a la Gamma. Este cambio de modelo sugiere que los mecanismos físicos de obstrucción y multitrayecto introducen efectos multiplicativos en el error, lo cual es consistente con la génesis de procesos log-normales.
...
\subsection{Conclusiones del Análisis de Distribución}

El análisis estadístico permite establecer tres conclusiones fundamentales para el diseño de algoritmos de posicionamiento:

\begin{enumerate}
\item \textbf{Rechazo de la hipótesis de normalidad:} El error de distancia UWB no es gaussiano. Asumir normalidad (como hacen los filtros de Kalman estándar o mínimos cuadrados ordinarios) subestimará sistemáticamente la probabilidad de grandes errores, especialmente en NLOS.
\item \textbf{Modelos dependientes del contexto:} La mejor descripción probabilística cambia con el entorno: Gamma para LOS y Log-normal (o Gamma con parámetros distintos) para NLOS.
\item \textbf{Implicación para algoritmos robustos:} Los algoritmos de fusión de datos deberían incorporar modelos de error de cola pesada o mecanismos adaptativos que ajusten la varianza del modelo en función de la detección de condiciones NLOS.
\end{enumerate}
