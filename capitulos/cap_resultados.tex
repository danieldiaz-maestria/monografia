...
\section{Análisis de la Distribución de Probabilidad del MAE}
\label{sec:analisis_distribucion_mae}

Con el objetivo de caracterizar formalmente el comportamiento estocástico del error de medición y validar la hipótesis de no normalidad sugerida en las secciones anteriores, se realizó un análisis de ajuste de distribuciones probabilísticas sobre el conjunto total de datos experimentales ($N=101,552$ muestras). Se evaluaron seis distribuciones teóricas candidatas: Normal, Log-normal, Gamma, Weibull, Exponencial y Uniforme. El ajuste se determinó mediante el Criterio de Información de Akaike (AIC) y el Criterio de Información Bayesiano (BIC), donde valores menores indican un mejor compromiso entre la fidelidad del ajuste y la complejidad del modelo.
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.8\linewidth]{imagenes/ajuste_hist_Todos_los_Datos.png}
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.8\linewidth]{imagenes/ajuste_hist_Datos_LOS.png}
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.8\linewidth]{imagenes/ajuste_hist_Datos_NLOS.png}
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.8\linewidth]{imagenes/ajuste_qq_Datos_NLOS.png}
...
Con base en el análisis de ajuste, se determinaron los parámetros óptimos para los modelos probabilísticos que describen el error de medición. Antes de presentar los valores específicos, se definen las tres distribuciones teóricas seleccionadas y la interpretación física de sus parámetros:

\begin{itemize}
    \item \textbf{Distribución Gamma:} 
    \begin{equation}
    f(x) = \frac{x^{\alpha-1} e^{-x/\theta}}{\theta^\alpha \Gamma(\alpha)}
    \end{equation}
    Donde $\alpha$ (parámetro de forma) define la curtosis de la distribución y $\theta$ (parámetro de escala) modula la dispersión de la cola.
    
    \item \textbf{Distribución Weibull:}
    \begin{equation}
    f(x) = \frac{k}{\lambda} \left(\frac{x}{\lambda}\right)^{k-1} e^{-(x/\lambda)^k}
    \end{equation}
    Donde $k$ es el parámetro de forma y $\lambda$ el parámetro de escala. Un $k < 1$ indica una tasa de probabilidad decreciente.
    
    \item \textbf{Distribución Log-normal:}
    \begin{equation}
    f(x) = \frac{1}{x \sigma \sqrt{2\pi}} \exp\left(-\frac{(\ln x - \mu)^2}{2\sigma^2}\right)
    \end{equation}
    Donde $\mu$ y $\sigma$ representan la media y desviación estándar del logaritmo natural de la variable, respectivamente.
\end{itemize}

Es crucial detallar la metodología utilizada para la obtención de estos parámetros a partir de los datos crudos. El ajuste no se basó en el método de momentos (igualación simple de medias y varianzas), sino en el método de \textbf{Máxima Verosimilitud (Maximum Likelihood Estimation - MLE)}. Este algoritmo busca iterativamente los valores de los parámetros que maximizan la probabilidad matemática de observar la muestra experimental exacta, garantizando un ajuste óptimo incluso en distribuciones con colas pesadas.

Conceptualmente, sin embargo, los parámetros conservan una relación física directa con la estadística descriptiva de los datos reales:
\begin{itemize}
    \item En el caso \textbf{Gamma}, $\alpha$ y $\theta$ dependen de la relación señal-ruido inherente a los datos ($E[x]^2 / Var[x]$). Una varianza alta en comparación con la media resulta en un parámetro de forma $\alpha$ bajo.
    \item En el caso \textbf{Log-normal}, los parámetros $\mu$ y $\sigma$ corresponden exactamente al promedio y la desviación estándar de los datos transformados logarítmicamente ($y = \ln(x)$).
    \item En el caso \textbf{Weibull}, el parámetro de escala $\lambda$ representa un valor de error característico, mientras que la forma $k$ modela la pendiente de la distribución.
\end{itemize}
...
Para ubicaciones críticas como la cadera y el pecho, los modelos ajustados revelan comportamientos particulares. En NLOS, tanto cadera como pecho se ajustan mejor a distribuciones Gamma con parámetros de forma $\alpha > 2.5$, lo que indica una distribución más acampanada y desplazada hacia la derecha en comparación con el caso LOS. Sorprendentemente, para el caso LOS en estas ubicaciones, la distribución Weibull ofrece un mejor ajuste que la Gamma, sugiriendo que la obstrucción parcial del propio cuerpo altera la naturaleza del error incluso cuando existe línea de vista geométrica.
