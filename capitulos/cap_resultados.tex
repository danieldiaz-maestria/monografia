...
\begin{figure}[ht]
    \centering
    % \includegraphics[width=0.8\textwidth]{imagenes/error_pecho_vs_orientacion.pdf}
    \caption{Error de Distancia vs. Ángulo de Orientación del Cuerpo - Dispositivo en Pecho}
    \label{fig:error_pecho_orientacion}
\end{figure}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Condición} & \textbf{ME (cm)} & \textbf{Desv. Est. (cm)} & \textbf{RMSE (cm)} & \textbf{Percentil 95 (cm)} \\
\midrule
LOS (0°-45°) & X.X & X.X & X.X & X.X \\
NLOS (45°-135°) & X.X & X.X & X.X & X.X \\
NLOS (135°-225°) & X.X & X.X & X.X & X.X \\
NLOS (225°-315°) & X.X & X.X & X.X & X.X \\
...
[COMPLETAR: Gráfica comparativa mostrando el error medio para cada ubicación corporal en función del ángulo de orientación del cuerpo]
...
Para cada combinación de ubicación corporal y condición de propagación (LOS/NLOS), se ajustaron distribuciones de probabilidad teóricas a los errores observados.
...
Orientación del cuerpo & XXX.X & X & XXX.X & XX.X & < 0.001 \\
Distancia & XXX.X & X & XXX.X & XX.X & < 0.001 \\
Ubicación × Orientación & XXX.X & X & XXX.X & XX.X & < 0.001 \\
...
Los resultados del ANOVA indican que [COMPLETAR: todos los factores principales tienen efecto significativo (p < 0.001), así como la interacción entre ubicación corporal y ángulo de orientación, sugiriendo que el patrón de variación del error con la orientación depende de dónde se porte el dispositivo].
...
\begin{tabular}{lcccc}
\toprule
\textbf{Variable} & \textbf{LOS} & \textbf{NLOS} & \textbf{p-valor} \\
\midrule
Estatura & 0.XX & 0.XX & X.XXX \\
Peso & 0.XX & 0.XX & X.XXX \\
IMC & 0.XX & 0.XX & X.XXX \\
\bottomrule
\end{tabular}
\end{table}
